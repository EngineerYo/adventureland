	...
	function render_slot(slot, shade, op) {
		...
		if (player.slots[slot]) {
			...
			chtml += item_container({
				...
				sname: player.me && slot, shade: shade, s_op: op, slot: slot
			}, current); // num:slot is new [06/08/16]
		}
	...
	for (var i = 0; i < Math.ceil(max(character.isize, items.length) / 7); i++) {
		...
		for (var j = 0; j < 7; j++) {
			...
			if (current) {
				...
				html += item_container({
					skin: skin,
					def: item, id: "str" + id, draggable: true, strnum: last - 1, snum: last - 1
				}, current);
		...
		for (var j = 0; j < 5; j++) {
			if (!type && i == 3 && j == 0 && s_page != 0) html += item_container({ skin: "left", onclick: "s_page=" + (s_page - 1) + "; render_secondhands();" }, { q: s_page, left: true });
				...
				html += item_container({
					skin: item.skin,
					onclick: f + "(" + (last - 1) + ")",
					def: item, id: id, draggable: false, droppable: false
				}, current);
			}
			else {
